          ...
          datasets: [
            {
              label: 'HbA1c',
              data: [
                patient.hba1c_1st_visit,
                patient.hba1c_2nd_visit,
                patient.hba1c_3rd_visit,
              ],
              borderColor: '#6366f1',
          ...
          {patient && (
            <span
              className={`inline-block text-xs px-3 py-1 rounded-full ${
                getStatusTag(patient) === 'Improving'
          ...
          <div className="mt-6">
            ...
            <div
              className={`rounded-lg p-4 shadow-sm border-l-4 ${
                patient.remarks ? 'bg-yellow-50 border-yellow-400' : 'bg-gray-50 border-gray-300'
              }`}
            >
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm text-gray-700 mt-4">
            <div>
              <strong>HbA1c Drop/Day:</strong> {patient.reduction_a_per_day?.toFixed(3)}
            </div>
            <div>
              <strong>Avg FVG:</strong> {patient.avg_fvg_1_2}
            </div>
            <div>
              <strong>Gap (1st→3rd):</strong> {Math.round(patient.gap_from_initial_visit)} days
            </div>
            <div>
              <strong>Gap (2nd→3rd):</strong> {Math.round(patient.gap_from_first_clinical_visit)} days
            </div>
          </div>
        </div>
