...
const RiskDashboard = () => {
  ...
  const runPredictions = async (data) => {
    ...
    for (const patient of data) {
      ...
      try {
        const res = await axios.post('http://localhost:8001/predict', { features });
        const value = parseFloat(res.data.prediction).toFixed(2);
        const label = mapNumericRisk(parseFloat(value));
